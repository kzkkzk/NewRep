...
var resize_blc = function(){
    ...
    var d_diamond = square_element.height();                       /*Äèàãîíàëü ðîìáà*/

    var w_diamond = d_diamond / Math.sqrt(2);                      /*Ñòîðîíà ðîìáà*/
    var delta_diamond = (square_element.height() - w_diamond)/2;   /*Âåëè÷èíà ñìåùåíèÿ*/

    diamond_element.css({ 'left': delta_diamond, 'top': delta_diamond, 'right': delta_diamond, 'bottom': delta_diamond});



    var h_gtop    = Math.sqrt( Math.pow((w_diamond/2), 2) - (Math.pow(square_element.height()/2, 2) /4)); /*âûñîòà ðàâíîáåäðåííîãî òðåóãîëüíèêà*/
...
/* ÏÎÑÒÐÎÅÍÈÅ ÌÅÍÞ */
    ...
    if (description_window_en == 0)                                     /* ÎÁÛ×ÍÎÅ ÑÎÑÒÎßÍÈÅ ÌÅÍÞ */
    {
